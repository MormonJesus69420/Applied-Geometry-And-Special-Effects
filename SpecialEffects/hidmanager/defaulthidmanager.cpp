...
void DefaultHidManager::heLockTo(const HidInputEvent::HidInputParams& params)
{
  ...
  if (sel_obj)
    ...
    cam->lock(
        (scene()->getSphereClean().getPos() - cam->getPos()) * cam->getDir());
  }
}
...
void DefaultHidManager::heMoveSelectedObjects(const HidInputEvent::HidInputParams& params)
{
  ...
  for (int i = 0; i < sel_objs.getSize(); i++) {
    ...
    const Vector<float, 3> deltav(
        ((prev(0) - pos(0)) * dh) * cam->getSide() + ((pos(1) - prev(1)) * dh) * cam->getUp());
...
void DefaultHidManager::heRotateSelectedObjects(const HidInputEvent::HidInputParams& params)
{
  ...
  const UnitVector<float, 3> rot_v = float(pos(0) - prev(0)) * cam->getGlobalUp() - float(prev(1) - pos(1)) * cam->getGlobalSide();
  const Angle ang(
      M_2PI * sqrt(pow(double(pos(0) - prev(0)) / cam->getViewportW(), 2) + pow(double(prev(1) - pos(1)) / cam->getViewportH(), 2)));
...
void DefaultHidManager::heSelectObjects(const HidInputEvent::HidInputParams& params)
{
  ...
  if (auto obj = findSceneObject(view_name, pos))
    obj->toggleSelected();
...
void DefaultHidManager::setupDefaultHidBindings()
{
  ...
  // Camera
  QString ha_id_view_move_camera = registerHidAction("View",
      "Move Camera",
      "Move the camera. "
  ...
  QString ha_id_view_pan_h = registerHidAction("View",
      "Pan Horizontally",
      "Pan horizontally",
      this, SLOT(hePanHorizontal(HidInputEvent::HidInputParams)));

  QString ha_id_view_pan_v = registerHidAction("View",
      "Pan Vertically",
      "Pan vertically",
      this, SLOT(hePanVertical(HidInputEvent::HidInputParams)));

  QString ha_id_view_zoom = registerHidAction("View",
      "Zoom",
      "Zoom",
      this, SLOT(heZoom(HidInputEvent::HidInputParams)));

  QString ha_id_view_lock_to = registerHidAction("View",
      "Lock To ...",
  ...
  QString ha_id_objtrans_scale = registerHidAction("Object transformation",
      "Scale Objects",
      "Scale objects",
      this, SLOT(heScaleSelectedObjects(HidInputEvent::HidInputParams)));

  QString ha_id_objtrans_move = registerHidAction("Object transformation",
      "Move Objects",
      "Move objects",
      this, SLOT(heMoveSelectedObjects(HidInputEvent::HidInputParams)),
      OGL_TRIGGER);

  QString ha_id_objtrans_rotate = registerHidAction("Object transformation",
      "Rotate Objects",
      "Rotate objects",
  ...
  QString ha_id_objsel_select_multi = registerHidAction("Object selection",
  ...
  // Object Interaction
  QString ha_id_objint_toggle_colaps = registerHidAction("Object interaction",
      "Toggle: Colapsed mode",
      "Toggle colapsed mode for objects",
      this, SLOT(heColapse()),
      OGL_TRIGGER);

  QString ha_id_objint_toggle_close = registerHidAction("Object interaction",
      "Toggle: Close mode",
      "Toggle close mode for objects",
      this, SLOT(heClose()),
      OGL_TRIGGER);

  QString ha_id_objint_toggle_selectors = registerHidAction("Object interaction",
      "Toggle: Selectors for objects",
      "Toggle selectors for objects",
      this, SLOT(heSelectors()),
      OGL_TRIGGER);

  QString ha_id_objint_toggle_edit = registerHidAction("Object interaction",
      "Toggle: Edit mode",
      "Toggle edit mode for editable objects",
      this, SLOT(heEdit()),
      OGL_TRIGGER);

  QString ha_id_objint_replot_high = registerHidAction("Object interaction",
      "Replot: QuickHigh",
      "Replot with \"high\" resolution",
      this, SLOT(heReplotQuickHigh()),
      OGL_TRIGGER);

  QString ha_id_objint_replot_med = registerHidAction("Object interaction",
      "Replot: QuickMedium",
      "Replot with \"medium\" resolution",
      this, SLOT(heReplotQuickMedium()),
      OGL_TRIGGER);

  QString ha_id_objint_replot_low = registerHidAction("Object interaction",
      "Replot: QuickLow",
  ...
  // Rendering
  QString ha_id_render_toggle_shademode = registerHidAction("Rendering",
      "Toggle: Shading mode",
  ...
  // Simulator
  QString ha_id_sim_toggle = registerHidAction("Simulator",
      "Toggle: Simulation",
      "Toggle simulation",
      this, SLOT(heToggleSimulation()));

  // Open/Close HidBindings view
  QString ha_id_var_open_close_hbview = registerHidAction("Application",
      "Open/Close Hid help",
      "Toggle open/close the Hid bindings help view",
      this, SLOT(heOpenCloseHidHelp()));

  // Various cleanup
  QString ha_id_var_lm_rel = registerHidAction("Various",
      "Left Mouse Release",
